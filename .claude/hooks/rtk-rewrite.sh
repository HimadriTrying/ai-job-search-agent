...
#
# Vendored from https://github.com/rtk-ai/rtk (hooks/claude/rtk-rewrite.sh).
# Local change: prepend the install dirs used by .claude/hooks/session-start.sh
# (~/.local/bin for the release binary, ~/.cargo/bin for the cargo build) so the
# binary is found in web/remote sessions.
#
